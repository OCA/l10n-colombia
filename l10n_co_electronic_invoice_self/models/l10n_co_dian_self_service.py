...
class L10nCoDianSelfService(models.AbstractModel):
    ...
    def _generate_signed_xml(self, move):
        ...
        try:
            ...
            xml_content, errors = self._export_invoice_new(move)
            if errors:
            ...
            raise UserError(_("Error al generar XML firmado: %s") % str(e)) from e
    ...
    def _build_wcf_body(self, action_url, filename, content_b64, journal):
        ...
        parts = [
            ...
            f"<wcf:contentFile>{content_b64}" f"</wcf:contentFile>",
        ]
    ...
    def _send_to_dian(self, journal, soap_data):
        ...
        if not url:
            raise UserError(_("No se encontró URL DIAN para el modo '%s'") % mode)
    ...
    def _query_status(self, move):
        ...
        if not track_id:
            raise UserError(_("No hay CUFE ni TrackId para consultar."))
        ...
        get_status_body = (
            "<wcf:GetStatus>"
            f"<wcf:trackId>{track_id}</wcf:trackId>"
            "</wcf:GetStatus>"
        )
    ...
    def _query_zip_status(self, move):
        ...
        get_status_body = (
            "<wcf:GetStatusZip>"
            f"<wcf:trackId>{zip_key}</wcf:trackId>"
            "</wcf:GetStatusZip>"
        )
        soap = self._build_soap_envelope(
            move,
    ...
    def _get_address_node(self, vals):
        ...
        return {
            "cbc:ID": {"_text": str(partner.city_id.zipcode).zfill(5)},
        ...
        return {
            ...
            "cac:PartyLegalEntity": {
                ...
                "cac:CorporateRegistrationScheme": {
                    "cbc:ID": {
                        "_text": (
                            invoice.journal_id._get_electronic_document_numbering_range()[
                                0
                            ]
...
Concepto de: {base_line['product_id'].name}"
    ...
    def _fill_dian_data(self, xml_content, record):
        ...
        total_element = root.find(
            f".//{{{NS_CAC_FULL}}}LegalMonetaryTotal" f"/{{{NS_CBC_FULL}}}PayableAmount"
        )
