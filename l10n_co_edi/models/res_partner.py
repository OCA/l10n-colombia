...
class ResPartner(models.Model):
    ...
    type_liability_id = fields.Many2one(
        comodel_name="l10n_co_edi_jorels.type_liabilities",
    ...
    customer_software_id = fields.Many2one(
        comodel_name="l10n_co_edi_jorels.customer_software",
        string="Customer software",
        copy=False,
        ondelete="RESTRICT",
    )
    type_document_identification_id = fields.Many2one(
        ...
        readonly=True,
        compute="_compute_type_document_identification_id",
        store=True,
        copy=False,
        ondelete="RESTRICT",
    )
    ...
    postal_id = fields.Many2one(
        comodel_name="l10n_co_edi_jorels.postal",
        copy=True,
        string="Postal",
        compute="_compute_postal_id",
        store=True,
    )
    postal_department_id = fields.Many2one(
    ...
    postal_municipality_id = fields.Many2one(
        comodel_name="l10n_co_edi_jorels.municipalities",
        copy=True,
        string="Postal municipality",
        compute="_compute_postal_id",
        store=True,
    )

    edi_sanitize_vat = fields.Char(
        "Sanitized vat", compute="_compute_edi_sanitize_vat", store=True, readonly=True
    )
    ...
    def _compute_edi_sanitize_vat(self):
        ...
        for rec in self:
            rec.edi_sanitize_vat = rec._edi_sanitize_vat(
                rec.vat, rec.type_document_identification_id.id
            )

    @api.depends("l10n_latam_identification_type_id")
    def _compute_type_document_identification_id(self):
        if not self.env[
            "l10n_co_edi_jorels.type_document_identifications"
        ].search_count([]):
        ...
        for rec in self:
            if rec.zip and rec.country_id and rec.country_id.code == "CO":
                ...
                if postal_search:
                    ...
                    rec.postal_department_id = (
                        rec.env["l10n_co_edi_jorels.departments"]
                        .sudo()
                        .search([("code", "=", rec.postal_id.department_id.code)])[0]
                        .id
                    )
                    rec.postal_municipality_id = (
                        rec.env["l10n_co_edi_jorels.municipalities"]
                        .sudo()
                        .search([("code", "=", rec.postal_id.municipality_id.code)])[0]
                        .id
                    )
            else:
                rec.postal_id = None
