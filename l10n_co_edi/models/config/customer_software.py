...
class CustomerSoftware(models.Model):
    ...
    receipt_event_field = fields.Selection(
    ...
    rejection_event_field = fields.Selection(
        [("subject", "Subject"), ("body", "Body")],
        "Field",
        default="subject",
        required=True,
    ...
    acceptance_event_field = fields.Selection(
        [("subject", "Subject"), ("body", "Body")],
        "Field",
        default="subject",
        required=True,
    ...
    def check_receipt(self, msg_dict):
        ...
        return (
            msg_dict[event_field].find(ef) != -1
    ...
    def check_acceptance(self, msg_dict):
        ...
        es = (
            self.acceptance_event_startswith if self.acceptance_event_startswith else ""
        )
    ...
    def get_move_id(self, mail_message):
        ...
        try:
            ...
            search_text = (
                ...
                if self.number_field == "subject"
                else mail_message.body
            )
            result = re.search(
                number_before + "(.*)" + number_after, search_text
            ).group(1)
            for res in result.split(" "):
                if res:
                    invoice_number = res
                    invoice_rec = self.env["account.move"].search(
                        [("number_formatted", "=", invoice_number)]
                    )[0]
