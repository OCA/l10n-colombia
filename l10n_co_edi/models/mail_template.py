...
class MailTemplate(models.Model):
    ...
    def generate_email(self, res_ids, fields):
        ...
        if self.model == "account.move":
            for move in records:
                inv_default_template = self.env.ref(
                ...
                if self.id in (ei_template.id, inv_default_template.id):
                    ...
                    attachments = (
                        res_t["attachments"]
                        if move.company_id.ei_include_pdf_attachment
                        else []
                    )
                    ...
                    if move.is_to_send_edi_email():
                        ...
                        xml_handle.write(
                            base64.decodebytes(move.ei_attached_document_base64_bytes)
                        )
                        xml_handle.close()
                        ...
                        with open(zip_path, "rb") as f:
                            attached_zip = f.read()
                            ei_attached_zip_base64_bytes = base64.encodebytes(
                                attached_zip
                            )
            ...
            for radian in records:
                ...
                attachments = (
                    res_t["attachments"]
                    if radian.company_id.ei_include_pdf_attachment
                    else []
                )
                # attachments = []
                ...
                if (
                    ...
                    xml_handle = open(xml_path, "wb")
                    xml_handle.write(
                        base64.decodebytes(radian.edi_attached_document_base64)
                    )
                    xml_handle.close()
                    zip_archive.write(xml_path, arcname=xml_name)
