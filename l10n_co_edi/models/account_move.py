...
class AccountMove(models.Model):
    ...
    ei_type_document_id = fields.Many2one(
        comodel_name="l10n_co_edi_jorels.type_documents",
    ...
    ei_is_correction_without_reference = fields.Boolean(
        "Is it a correction without reference?",
        default=False,
        readonly=True,
        states={"draft": [("readonly", False)]},
    )
    ...
    resolution_id = fields.Many2one(
        string="Resolution",
        comodel_name="l10n_co_edi_jorels.resolution",
        copy=False,
        store=True,
                ...
                if rec.partner_id.vat:
                    ...
                    if identification_number:
                        ...
                        if rec_partner.country_id:
                            customer_rec = self.env[
                                "l10n_co_edi_jorels.countries"
                            ].search([("code", "=", rec_partner.country_id.code)])
                            if customer_rec:
                        ...
                        if invoice_line_tax_id.edi_tax_id.id:
                            ...
                            if (
                                ...
                                if invoice_line_tax_id.amount_type == "percent":
                                    ...
                                    tax_total.update(
                                        {
                                            "tax_value": invoice_line_id.quantity
                                            * invoice_line_tax_id.amount
                                        }
                                    )
                                    tax_total.update(
                                    ...
                                    if commercial_sample:
                                        ...
                                        tax_total.update(
                                            {"tax_value": invoice_line_id.price_total}
                                        )
                                        tax_total.update(
                ...
                if type_edi_document != "none":
                    if rec.ei_zip_key or rec.ei_uuid:
                        ...
                        api_url = (
                            ...
                            .sudo()
                            .get_param(
                                "jorels.edipo.api_url", "https://edipo.jorels.com"
                            )
                        )
                        params = {
                            "token": token,
