...
class Radian(models.Model):
    ...
    note = fields.Text(
        string="Note", readonly=True, states={"draft": [("readonly", False)]}
    )
    rejection_concept_id = fields.Many2one(
        comodel_name="l10n_co_edi_jorels.rejection_concepts",
    ...
    edi_is_valid = fields.Boolean(
        "Is valid?", copy=False, readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    edi_uuid = fields.Char(
        "UUID", copy=False, readonly=True, states={"draft": [("readonly", False)]}
    )
    edi_issue_date = fields.Date(
        "Date", copy=False, readonly=True, states={"draft": [("readonly", False)]}
    )
    edi_expedition_date = fields.Char("Expedition date", copy=False, readonly=True)
    edi_zip_key = fields.Char(
        "Zip key", copy=False, readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    edi_pdf_base64 = fields.Binary(
        "PDF", copy=False, readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    def _send_email(self):
        for rec in self:
            mail_template = rec.env.ref(
                "l10n_co_edi_jorels.email_template_radian", False
            )
