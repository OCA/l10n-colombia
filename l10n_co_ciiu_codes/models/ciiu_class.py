...
class CiiuClass(models.Model):
    _description = "CIIU Class"
    _name = "apciu.class"
    ...
    apciu_group_id = fields.Many2one('apciu.group', string='Group', required='True')
    ...
    @api.onchange('apciu_division_id')
    def _onchange_apciu_division_id(self):
        if not self.apciu_division_id:
            self.apciu_group_id = False
        else:
            if self.apciu_group_id and self.apciu_group_id.apciu_division_id != self.apciu_division_id:
                self.apciu_group_id = False
        if self.apciu_division_id:
            return {'domain': {'apciu_group_id': [('apciu_division_id', '=', self.apciu_division_id.id)]}}
        else:
            return {'domain': {'apciu_group_id': []}}
